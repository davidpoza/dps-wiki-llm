# explorer-graph-view Specification

## Purpose
Provides an interactive graph view of the vault's note connections inside the explorer. The user can toggle the graph panel via a toolbar button; the main area then renders a Cytoscape.js force-directed graph with all vault notes as nodes and wikilinks as edges. A settings panel (filter, orphan toggle, physics sliders) replaces the file tree while the graph is active.
