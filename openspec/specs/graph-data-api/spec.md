...
## Purpose
Exposes a JWT-protected `GET /api/graph` endpoint that scans all vault markdown files, resolves `[[wikilink]]` references, and returns a structured graph of nodes (notes) and edges (resolved wikilinks) for consumption by the frontend graph view.
