...
### Requirement: Umbral de link discovery configurable

El servicio de descubrimiento de enlaces SHALL usar el umbral almacenado en `app_settings` (key `link.similarity-threshold`) en lugar de un valor hardcodeado, con fallback a 0.72.

#### Scenario: Umbral configurado en Settings se aplica en link discovery

- **WHEN** el usuario ha configurado `link.similarity-threshold = 0.80` en Settings
- **AND** abre el modal de Link Discovery para una nota
- **THEN** solo se muestran candidatos con score >= 0.80
