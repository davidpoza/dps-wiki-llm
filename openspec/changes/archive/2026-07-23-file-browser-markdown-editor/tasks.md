...
- [x] 6.1 Verificar que `GET /api/files/tree` responde con el árbol jerárquico y requiere JWT
- [x] 6.2 Verificar que `GET /api/files/content?path=../../../etc/passwd` retorna HTTP 400
- [x] 6.3 Verificar flujo completo: navegar a `/explorer`, seleccionar fichero, editar, guardar, recargar y ver cambio persistido
- [x] 6.4 Verificar que navegar a `/explorer` sin autenticación redirige a `/login`
- [x] 6.5 Verificar diálogo de confirmación al cambiar de fichero con cambios sin guardar
