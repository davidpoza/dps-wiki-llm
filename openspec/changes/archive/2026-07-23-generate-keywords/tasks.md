...
- [x] 8.2 Manually run the flow: click "Generar keywords", confirm SSE progress updates live, that only notes missing `keywords` are updated, that keyword text comes from `Summary` (or body minus excluded sections), and that edited notes show minimal diffs in history and sync to WebDAV
