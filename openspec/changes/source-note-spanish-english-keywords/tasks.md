...
- [ ] 4.1 Start the backend so Flyway applies `V25`; query `llm_prompts` for `source-note-system` and confirm the new text is stored.
- [ ] 4.2 Ingest one English and one Spanish source; confirm the generated `wiki/sources/**` note body is in Spanish and the frontmatter `keywords` are English, singular, lowercase, kebab-case (5–15 terms).
