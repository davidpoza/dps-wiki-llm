          ...
          handleTextInput: (view, from, _to, text) => {
            ...
            if (text === ')') {
              ...
              if (match) {
                ...
                return applyLinkMark(view, state, from, from - (fullMatchLen - 1), match[1], match[2], '');
              }
            }
            ...
            if (text === '>') {
              ...
              if (urlMatch) {
                ...
                return applyLinkMark(view, state, from, from - (fullMatchLen - 1), url, url, '');
              }
              const emailMatch = AUTOLINK_EMAIL_PATTERN.exec(textBefore);
              if (emailMatch) {
                const email = emailMatch[1];
                const fullMatchLen = emailMatch[0].length;
                return applyLinkMark(view, state, from, from - (fullMatchLen - 1), email, `mailto:${email}`, '');
              }
            }
