...
export function serializeMarkToMarkdown(mark: Mark, text: string): string {
  ...
  switch (mark.type.name) {
    case 'link':
      return a['title'] ? `[${text}](${a['href']} "${a['title']}")` : `[${text}](${a['href']})`;
    case 'strong':
      return `**${text}**`;
    case 'emphasis':
      return `_${text}_`;
...
function findTableAtCursor(state: EditorState): TableRange | null {
  const sel = state.selection;
  const pos =
    sel instanceof TextSelection && sel.$cursor ? sel.$cursor.pos : sel instanceof NodeSelection ? sel.from : -1;
    ...
    () =>
      new Plugin<LPState>({
        ...
        appendTransaction: (trs, _oldState, newState): Transaction | null => {
          ...
          const lastMeta =
            trs.length > 0 ? (trs[trs.length - 1].getMeta(META_KEY) as { type: string } | undefined) : undefined;
          ...
          const cursorPos =
            sel instanceof TextSelection && sel.$cursor
              ? sel.$cursor.pos
          ...
          if (lpState.expanded) {
            ...
            const effectiveTo =
              mode === 'block'
                ? (() => {
                    const n = newState.doc.nodeAt(from);
                    return n ? from + n.nodeSize : to;
                  })()
                : to;
        ...
        props: {
          handlePaste(view, event) {
            ...
            try {
              tr.setSelection(TextSelection.near(tr.doc.resolve(Math.min(afterPos, tr.doc.content.size))));
            } catch {
              /* noop */
            }
            view.dispatch(tr);
          ...
          handleKeyDown(view, event) {
                ...
                if (blockSel instanceof TextSelection && blockSel.$cursor) {
                  ...
                  if (expandedNode) {
                    ...
                    if (cursorPos > from && cursorPos < effectiveTo) {
                      ...
                      const textBefore =
                        cursorPos > innerFrom ? state.doc.textBetween(innerFrom, cursorPos, '', '') : '';
                      const textAfter = cursorPos < innerTo ? state.doc.textBetween(cursorPos, innerTo, '', '') : '';
            ...
            try {
              tableTr.setSelection(
                TextSelection.near(tableTr.doc.resolve(Math.min(tableAfterPos, tableTr.doc.content.size))),
              );
            } catch {
              /* noop */
            }
