      ...
      <aside class="chat-sidebar" [class.open]="sidebarOpen()">
        ...
        <div class="session-list">
          ...
          @for (s of sessions(); track s.id) {
            <div class="session-item" [class.active]="s.id === activeSessionId()" (click)="selectSession(s.id)">
...
export class ChatComponent implements OnInit, AfterViewChecked {
    ...
    this.chatService.sendMessage(sessionId, content).subscribe({
      next: (newMessages) => {
        ...
        if (session?.title === 'Nueva conversación' && newMessages.length > 0) {
          ...
          if (firstUserMsg) {
            const newTitle =
              firstUserMsg.content.length <= 60 ? firstUserMsg.content : firstUserMsg.content.substring(0, 60);
          ...
          this.sessions.update((list) => {
            const updated = list.map((s) => (s.id === sessionId ? { ...s, updatedAt: new Date().toISOString() } : s));
  ...
  exportToVault(): void {
    ...
    this.chatService.exportToVault(sessionId).subscribe({
      next: (res) => {
        this.exportPath.set(res.path);
        this.exportLoading.set(false);
      },
      error: () => {
        this.exportError.set('Error al guardar');
        this.exportLoading.set(false);
      },
    });
  }
