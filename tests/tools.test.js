...
test("ingest workflow auto-applies non-empty LLM mutation plans with guardrails", async () => {
  ...
  assert.equal(nodes.get("Call OpenRouter Source Note Cleaner").type, "n8n-nodes-base.code");
  assert.equal(nodes.get("Call OpenRouter Ingest Planner").type, "n8n-nodes-base.code");
  assert.match(nodes.get("Call OpenRouter Source Note Cleaner").parameters.jsCode, /LLM_API_KEY_HEADER/);
  assert.match(nodes.get("Call OpenRouter Ingest Planner").parameters.jsCode, /LLM_API_KEY_HEADER/);
  assert.match(nodes.get("Call OpenRouter Source Note Cleaner").parameters.jsCode, /Authorization/);
  assert.match(nodes.get("Call OpenRouter Source Note Cleaner").parameters.jsCode, /OPENROUTER_API_KEY/);
  assert.match(nodes.get("Call OpenRouter Source Note Cleaner").parameters.jsCode, /apiKeyValue/);
  assert.match(nodes.get("Call OpenRouter Source Note Cleaner").parameters.jsCode, /fetch/);
...
test("telegram bot workflow routes answer and ingest commands", async () => {
  ...
  assert.equal(nodes.get("Call OpenRouter Answer").type, "n8n-nodes-base.code");
  assert.equal(nodes.get("Call OpenRouter Feedback").type, "n8n-nodes-base.code");
  assert.equal(nodes.get("Call OpenRouter Ingest Planner").type, "n8n-nodes-base.code");
  assert.equal(nodes.get("Call OpenRouter Source Note Cleaner").type, "n8n-nodes-base.code");
  assert.match(nodes.get("Call OpenRouter Answer").parameters.jsCode, /LLM_API_KEY_HEADER/);
  assert.match(nodes.get("Call OpenRouter Feedback").parameters.jsCode, /LLM_API_KEY_HEADER/);
  assert.match(nodes.get("Call OpenRouter Ingest Planner").parameters.jsCode, /LLM_API_KEY_HEADER/);
  assert.match(nodes.get("Call OpenRouter Source Note Cleaner").parameters.jsCode, /LLM_API_KEY_HEADER/);
  assert.match(nodes.get("Call OpenRouter Answer").parameters.jsCode, /Authorization/);
  assert.match(nodes.get("Call OpenRouter Answer").parameters.jsCode, /OPENROUTER_API_KEY/);
  assert.match(nodes.get("Call OpenRouter Answer").parameters.jsCode, /apiKeyValue/);
  assert.match(nodes.get("Call OpenRouter Answer").parameters.jsCode, /fetch/);
