...
class MutationGuardrailServiceTests {
    ...
    private MutationGuardrailService guardrail() {
        AppProperties props = new AppProperties(vault.toString(), List.of("http://localhost:4200"),
    ...
    private MutationAction conceptAction(String path) {
        return new MutationAction(MutationActionType.create, path, "Title", Map.of("type", "concept"),
                Map.of("Sources", List.of("[[wiki/sources/source.md]]")), "key-1");
    }

    @Test
    void pluralSlugIsNormalizedToSingular() {
        MutationPlan plan = new MutationPlan("p1", List.of(conceptAction("wiki/concepts/mental-models.md")));
        ...
        assertThat(result.plan().pageActions().getFirst().path()).isEqualTo("wiki/concepts/mental-model.md");
        assertThat(result.plan().pageActions().getFirst().action()).isEqualTo(MutationActionType.create);
    }

    @Test
    void ambiguousPluralIsConvertedToNoop() {
        MutationPlan plan = new MutationPlan("p1", List.of(conceptAction("wiki/concepts/analyses.md")));
    ...
    void correctSingularSlugPassesThrough() {
        MutationPlan plan = new MutationPlan("p1", List.of(conceptAction("wiki/concepts/mental-model.md")));
    ...
    void iesEndingNormalizedToY() {
        MutationPlan plan = new MutationPlan("p1", List.of(conceptAction("wiki/concepts/design-strategies.md")));

        var result = guardrail().guardrail(plan, "raw/inbox/a.md", "wiki/sources/source.md");

        assertThat(result.rejections()).isEmpty();
        assertThat(result.plan().pageActions().getFirst().path()).isEqualTo("wiki/concepts/design-strategy.md");
    }

    @Test
    void nonConceptPathNotAffectedBySlugNormalization() {
        MutationAction action = new MutationAction(MutationActionType.create, "wiki/analyses/2024-report.md",
                "Report", Map.of("type", "analysis"),
                Map.of("Sources", List.of("[[wiki/sources/source.md]]")), "key-a");
        MutationPlan plan = new MutationPlan("p1", List.of(action));

        var result = guardrail().guardrail(plan, "raw/inbox/a.md", "wiki/sources/source.md");

        assertThat(result.plan().pageActions().getFirst().path()).isEqualTo("wiki/analyses/2024-report.md");
    }
}
