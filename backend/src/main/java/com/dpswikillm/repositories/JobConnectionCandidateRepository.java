...
public interface JobConnectionCandidateRepository
        extends JpaRepository<JobConnectionCandidate, UUID> {
    List<JobConnectionCandidate> findByJobIdOrderByCreatedAtAsc(UUID jobId);

    List<JobConnectionCandidate> findByJobIdAndDecision(
            UUID jobId, ConnectionCandidateDecision decision);
}
